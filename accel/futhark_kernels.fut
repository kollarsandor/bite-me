...
entry rsf_forward_f32 [half] (x: [half*2]f32) (s_w: [half][half]f32) (t_w: [half][half]f32) (s_b: [half]f32) (t_b: [half]f32) (perm: [half*2]i64): [half*2]f32 = rsf_forward_layer x s_w t_w s_b t_b perm
entry rsf_backward_f32 [half] (grad: [half*2]f32) (x: [half*2]f32) (s_w: [half][half]f32) (t_w: [half][half]f32) (s_b: [half]f32) (t_b: [half]f32) (perm: [half*2]i64): ([half*2]f32, [half][half]f32, [half][half]f32, [half]f32, [half]f32) = rsf_backward_layer grad x s_w t_w s_b t_b perm
...
entry batch_gradients [batch_size][seq_len][half] (inputs: [batch_size][seq_len][half*2]f16)
  ...
  let count = f16.i64 (batch_size * seq_len)
  let inv_count = f16.f64 1.0 f16./ count
  in (map (map (\v -> v f16.* inv_count)) gs_total,
      map (map (\v -> v f16.* inv_count)) gt_total,
      map (\v -> v f16.* inv_count) gsb_total,
      map (\v -> v f16.* inv_count) gtb_total)
