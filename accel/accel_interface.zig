...
pub const FutharkContext = struct {
    ...
    pub fn init(device_id: i32) AccelError!Self {
        ...
        const device_str = std.fmt.bufPrintZ(&buf, "{d}", .{device_id}) catch return AccelError.FutharkConfigFailed;
        futhark.futhark_context_config_set_device(cfg, device_str.ptr);
...
pub const FutharkArray1DF16 = struct {
    ...
    pub fn free(self: *Self, ctx: *FutharkContext) void {
        if (self.arr) |arr| {
            if (ctx.ctx) |c| {
                _ = futhark.futhark_free_f16_1d(c, arr);
            }
            self.arr = null;
            self.len = 0;
        }
...
pub const FutharkArray2DF16 = struct {
    ...
    pub fn free(self: *Self, ctx: *FutharkContext) void {
        if (self.arr) |arr| {
            if (ctx.ctx) |c| {
                _ = futhark.futhark_free_f16_2d(c, arr);
            }
            self.arr = null;
            self.rows = 0;
            self.cols = 0;
...
pub const FutharkArray2DF32 = struct {
    ...
    pub fn free(self: *Self, ctx: *FutharkContext) void {
        if (self.arr) |arr| {
            if (ctx.ctx) |c| {
                _ = futhark.futhark_free_f32_2d(c, arr);
            }
            self.arr = null;
            self.rows = 0;
            self.cols = 0;
...
pub const FutharkArray1DF32 = struct {
    ...
    pub fn free(self: *Self, ctx: *FutharkContext) void {
        if (self.arr) |arr| {
            if (ctx.ctx) |c| {
                _ = futhark.futhark_free_f32_1d(c, arr);
            }
            self.arr = null;
            self.len = 0;
        }
...
pub const RSFAccelerator = struct {
    ...
    pub fn forwardFromTensor(self: *Self, input: *const core_tensor.Tensor, allocator: std.mem.Allocator) AccelError!core_tensor.Tensor {
        ...
        defer allocator.free(f16_data);
        {
            ...
            const total = rows * cols;
            while (i < total) : (i += 1) {
                f16_data[i] = @floatCast(input.data[i]);
            }
        }
